...
function UnrealWebRTCPlayer(
  ...
  this.Play = function Play() {
    ...
    if (state == -1 && nowTime - latestStopTime > 1000) {
      ...
      remoteVideo.setAttribute(
        'style',
        `background: black url(${baseUrl}/images/loader.gif) center no-repeat;`,
      );
    ...
    if (nowTime - latestStopTime <= 1000) remoteVideo.pause();
  };
  ...
  function onCreateOfferSuccess(desc) {
    var audioRate = 8000;
    if (audioCodec === 'opus') audioRate = 48000;
    ...
    ws.onmessage = function (evt) {
      ...
      if (state == 0) {
        ...
        if (strArr.length == 1) {
          ...
          if (
            firstAttempt &&
            response === 'Error: Initialization of peer connection failed' &&
          ...
          serverEndpoint.candidate = EnsureValidCandidate(
  ...
  function setCodec(sdp, type, codec, clockRate) {
    ...
    for (var i = mLineIndex; i < sdpLines.length; i++) {
      if (sdpLines[i].search(codec + '/' + clockRate) !== -1) {
        ...
        if (
          codecPayload &&
          EnsureSupportedProfile(codec, sdpLines, mLineIndex, codecPayload)
        ) {
  ...
  function EnsureSupportedProfile(codec, sdpLines, mLineIndex, codecPayload) {
    ...
    for (var i = mLineIndex; i < sdpLines.length; i++) {
      if (
        ...
        sdpLines[i].search('profile-level-id=42') !== -1
      )
        return true;
    }
  ...
  function ToggleOverlay(show) {
    ...
    if (Overlay != null) {
      if (show) Overlay.style.display = 'inline';
